...
-- ANY, SOME(잘사용안하지만 같음): 값을 서브쿼리에 의해 리턴된 각각의 값과 비교해서
-- 하나라도 만족하면 조회 대상에 포함됩니다.
-- David라는 사람이 여러 명인데, 그중에 가장 적은 급여를 받는 David 보다 급여가 높은 사람을 조회.
-- 3중에 하나만 만족
SELECT * FROM employees
WHERE salary > ANY (SELECT salary FROM employees
                WHERE first_name = 'David'); -- (최솟값보다만 크면됨)
                
-- ALL: 값을 서브쿼리에 의해 리턴된 각각의 값과 모두 비교해서
-- 전부 다 일치해야 조회대상에 포함됩니다.
-- David라는 사람이 여러 명인데, 그중에 가장 많은 급여를 받는 David 보다 급여가 높은 사람을 조회.
SELECT * FROM employees
...
-- EXISTS 연산자가 1이 조회가 될때 데이터가 존재한다는 것을
-- 판단하여 employees에서 해당 사원에 모든 정보를 조회
