...
SELECT COUNT(first_name) FROM employees; -- 컬럼하나하는것보다 별찍이서 전체확인
...
-- 부서별로 그룹화, 그룹함수의 사용(GROUP BY절이 꼭 있어야한다.)
SELECT
    department_id,
    COUNT(employee_id)|| '명' AS  사원수,
    AVG(salary) AS 평균급여 
FROM employees
...
-- 일반 컬럼과 동시에 그냥 출력할 수는 없습니다. 그룹이 반드시 필요합니다.
SELECT
    department_id,
    AVG(salary) AS 평균급여
FROM employees
...
SELECT
    job_id, -- 그룹절에 묶이지 않은 컬럼이므로 조회불가능
    department_id,
    AVG(salary)
FROM employees
...
-- 부서 아이디가 50 이상인 것들을 그룹화 시키고, 그룹 급여평균이 5000 이상만 조회
SELECT 
    department_id,
    AVG(salary) AS 급여평균
FROM employees
...
SELECT
    ...
    TRUNC(AVG(salary + commission_pct*salary),2) AS 평균급여,
    SUM(salary + commission_pct*salary) AS 급여합계,
    COUNT(*) AS 사원수 
FROM employees 
