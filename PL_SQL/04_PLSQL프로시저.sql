...
-- job_id를 확인해서
-- 이미 존재하는 데이터라면 수정, 없다면 새롭게 추가(job_id가 PR 이기때문)
...
BEGIN
    ...
    -- 이미 존재한다면 1, 존재하지 않는다면 0 -> v_cnt에 전달
    SELECT 
        COUNT(*)
    INTO 
        v_cnt
...
EXEC my_new_job_proc('JOB5', 'test job5', 6800, 9600);
SELECT * FROM jobs;
...
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
     ...
     p_min_sal IN jobs.min_salary%TYPE := 0,
     p_max_sal IN jobs.max_salary%TYPE := 1000,
     p_result OUT VARCHAR2 -- 프로시저 내부에서 외부로 내보내기 위한 변수     
    )
IS
    v_cnt NUMBER := 0;
...
BEGIN
    ...
    -- OUT 매개변수에 조회 결과를 할당. -> 우라기 알고있는 return 개념과 같다.
    -- OUT 매개변수에 값을 할당해 놓으면 프로시저 종료 후 호출부로 OUT 변수의 값이 전달됨.
    p_result := v_result;
...
BEGIN
    ...
    my_new_job_proc('CEO2', 'test_CEO2', 30000, 80000);
    dbms_output.put_line(msg);
END;
...
CREATE OR REPLACE PROCEDURE my_param_test_proc
    ...
    -- IN: 반환 불가, 받는 용도로만 가능
    p_var1 IN VARCHAR2,
    -- OUT: 값 반환가능, 받는 용도로는 활용 불가능
    -- OUT이 되는 시점은 프로시저가 끝날 때 가능, 그전까지는 할당이 안됨
    p_var2 OUT VARCHAR2,
    -- IN OUT이 둘다 가능함.
...
BEGIN
    ...
    dbms_output.put_line('p_var2: ' || p_var2); -- OUT: 값을 전달했음에도 불구하고 프로시저 내에서 확인불가
    dbms_output.put_line('p_var3: ' || p_var3); -- IN OUT: IN의 성질을 가지고 있음 (출력됨)
...
-- RETURN: 값의 반환이 아니고(OUT 사용), 프로시저 강제 종료할 때 사용
CREATE OR REPLACE PROCEDURE my_new_job_proc
    (p_job_id IN jobs.job_id%TYPE,
    ...
    */
    ...
    EXCEPTION 
        WHEN ZERO_DIVIDE THEN
            dbms_output.put_line('0으로 나누시면 안됩니다!!!!');
            dbms_output.put_line('SQL ERROR CODE: ' || SQLCODE);
            dbms_output.put_line('SQL ERROR MSG: ' || SQLERRM);
        WHEN OTHER THEN
            -- WHEN으로 설정한 예외가 아닌 다른 예외발생 시 OTHERS 실행.
