...
-- AFTER 트리거
CREATE OR REPLACE TRIGGER trg_use_backup
    AFTER UPDATE OR DELETE
    ON tbl_user
    FOR EACH ROW -- 모든행에 적용
DECLARE
    v_type VARCHAR2(10);
BEGIN
    ...
    --  -> 원본 테이블에서 UPDATE or DELETE된 사용자의 정보 및 기타정보)
    -- USER():현재사용자의 계정명
    
    INSERT INTO tbl_user_backup
    VALUES(:OLD.id, :OLD.name, :OLD.address, sysdate, v_type, USER()); -- USER():현재사용자의 계정명
...
BEGIN
    -- INSERT 예정인 name의 값에서 첫글자(성)만 추출 후 뒤에 *을 세개 붙이겠다.
    :NEW.name := SUBSTR(:NEW.name, 1, 1) || '***';
END;

INSERT INTO tbl_user VALUES('test04', '사버미', '증평');
INSERT INTO tbl_user VALUES('test05', '가여니', '세종');

-- 트리거 내부에서는 롤백 불가, 바깥쪽에서는 가능하다
ROLLBACK;
...
CREATE SEQUENCE order_history_seq NOCYCLE NOCACHE; -- 시작1, 1씩증가
...
BEGIN 
    ...
    v_total := :NEW.total; -- 주문수량
    v_product_no := :NEW.product_no; -- 주문상품의 번호
    ...
    ELSIF v_total > v_product_total THEN -- 주문수량이 재고수량보다 많은경우
...
-- 같은주문에 다른물건 구매라서 200번(임의로)
INSERT INTO order_history VALUES(order_history_seq.NEXTVAL, 200, 1, 5, 50000);
INSERT INTO order_history VALUES(order_history_seq.NEXTVAL, 200, 2, 100, 20000);
