...
CREATE SEQUENCE dept2_seq 
    ...
    MAXVALUE 10 -- 최대값 (기본값은 증가일 때 1027, 감소일때 -1)
    MINVALUE 1 -- 최소값 (기본값은 증가일 때 1, 감소일때 -1028)
    NOCACHE -- 캐시 메모리 사용 여부 (CACHE)
    NOCYCLE; -- 순환 여부 (NOCYCLE이 기본, 순환시키려면 CYCLE)12345~,12345~ PK로 사용하려면 NOCYCLE 중복이 발생되면안되니까
...
-- (시퀀스 삭제 후 다시 생성하는게 낫다)
SELECT dept2_seq.CURRVAL FROM dual; -- 현재 시퀀스 값 확인

ALTER SEQUENCE dept2_seq INCREMENT BY -18;
