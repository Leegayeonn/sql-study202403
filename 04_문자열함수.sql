...
SELECT 
    salary
FROM employees
WHERE last_name = INITCAP('austin');
...
-- 인덱스는 1부터 시작
-- concat(문자연결): 매개값을 2개만 받음 (그 이상이라면 || 사용!)
...
-- 기존값 가릴때도 사용! (앞글자 3글자 제외 나머지 가리는등등,,)
SELECT
    LPAD('abc', 10, '*'),
    RPAD('abc', 10, '*')
FROM dual;
...
-- REPLACE(str, old, new)
SELECT
    REPLACE('javascript java', 'javascript','hello')
FROM dual;

SELECT
    REPLACE(REPLACE('javascript java', 'javascript',' hello'),' ','')
FROM dual;
