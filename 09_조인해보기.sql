...
AND e.job_id = j.job_id; -- 106명만 조회됨(null이 있어서 조건이 안맞음)
...
AND loc.state_province = 'California'; -- 일반조건은 조인조건 이후 마지막에 작성
...
WHERE e.department_id = d.department_id(+) -- 새로 갖다붙혀지는 쪽에(+)붙히기
AND d.location_id = loc.location_id(+);
/*
