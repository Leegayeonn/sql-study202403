...
-- 날짜로 문자로 형 변환할 때 원하는 형식으로 날짜를 표현할 수 있습니다.
SELECT TO_CHAR(sysdate)FROM dual;
SELECT TO_CHAR(sysdate, 'YYYY-MM-DD DY PM HH:MI:SS') FROM dual;
SELECT TO_CHAR(sysdate, 'YY-MM-DD HH24:MI:SS') FROM dual;


-- 서식문자와 함께 사용하고 싶은 문자를 ""로 감싸서 전달한다
-- 서식문자(-,:)가 아닌 일반문자를 포함하려면 쌍따옴표를 붙혀야한다.
SELECT 
TO_CHAR(sysdate, 'YYYY"년" MM"월" DD"일"') 
FROM dual;

-- 숫자를 문자로 TO_CHAR(값, 형식)
-- 자리수를 표현하려면(콤마표시)'9'를 이용하여 표시(실제 숫자 9가 아니고 자리수를 표현하는 기호)
SELECT TO_CHAR(20000)FROM dual;
SELECT TO_CHAR(20000, '99,999')FROM dual;
SELECT TO_CHAR(20000, 'L99,999')FROM dual; -- DB해당지역(국가)화폐로 표시
SELECT TO_CHAR(20000.2915, '99999.999')FROM dual; -- 반올림 지원
...
SELECT '2000' + 2000 FROM dual; -- 자동으로 형변환(문자 -> 숫자)
SELECT TO_NUMBER ('2000') + 2000 FROM dual; -- 명시적 형변환
SELECT '$3,300' + 2000 FROM dual; -- 에러
SELECT TO_NUMBER('$3,300', '$9,999') + 2000 FROM dual; -- 숫자와 문자를 구분해주면 가능
...
-- 조회 컬럼명은 dateInfo라고 하겠습니다. dual에서 조회
...
-- NULL 형태를 변환하는 함수 NVL(컬럼, 변환할 타겟값) -> NULL인 경우는 무시함
SELECT 
    first_name,
...
-- NULL 형태를 변환하는 함수 NVL2(컬럼, null이 아닐 경우의 값, null일 경우의값)
SELECT 
    first_name,
    salary,
    NVL2(
...
-- 오라클 전용 함수. 동등 비교만 가능(일치하는지만 비교가능!) 
SELECT
    first_name,
    job_id,
    salary,
